...
-- Projetos COMPLETED (datas no passado, end_date preenchida)
...
-- Projetos IN_PROGRESS (datas futuras, coerentes com 2026)
INSERT INTO tb_projects (title, description, client_id, status, start_date, expected_end_date, end_date, value)
SELECT 'Relatório Logístico', 'Automação de relatórios de entregas e rotas', 2, 'IN_PROGRESS', '2026-02-01', '2026-05-01', null, 15000.00
    WHERE NOT EXISTS (SELECT 1 FROM tb_projects WHERE title = 'Relatório Logístico');

INSERT INTO tb_projects (title, description, client_id, status, start_date, expected_end_date, end_date, value)
SELECT 'Sistema de Metas', 'Painel de metas e KPIs para equipe de vendas', 3, 'IN_PROGRESS', '2026-01-01', '2026-06-01', null, 12000.00
    WHERE NOT EXISTS (SELECT 1 FROM tb_projects WHERE title = 'Sistema de Metas');

INSERT INTO tb_projects (title, description, client_id, status, start_date, expected_end_date, end_date, value)
SELECT 'Agente IA Diagnóstico', 'Implementação de agente de IA para triagem de pacientes', 4, 'IN_PROGRESS', '2025-10-01', '2026-07-01', null, 45000.00
    WHERE NOT EXISTS (SELECT 1 FROM tb_projects WHERE title = 'Agente IA Diagnóstico');

INSERT INTO tb_projects (title, description, client_id, status, start_date, expected_end_date, end_date, value)
SELECT 'Automação Industrial', 'Automação de processos com análise preditiva', 5, 'IN_PROGRESS', '2025-11-01', '2026-08-01', null, 55000.00
    WHERE NOT EXISTS (SELECT 1 FROM tb_projects WHERE title = 'Automação Industrial');

-- Projetos DELAYED (expected_end_date no passado, status DELAYED)
INSERT INTO tb_projects (title, description, client_id, status, start_date, expected_end_date, end_date, value)
SELECT 'BI Financeiro', 'Análise financeira com indicadores de desempenho', 2, 'DELAYED', '2025-06-01', '2025-12-01', null, 22000.00
    WHERE NOT EXISTS (SELECT 1 FROM tb_projects WHERE title = 'BI Financeiro');

INSERT INTO tb_projects (title, description, client_id, status, start_date, expected_end_date, end_date, value)
SELECT 'Análise de Estoque', 'Monitoramento de estoque em tempo real', 3, 'DELAYED', '2025-07-01', '2025-11-01', null, 19000.00
    WHERE NOT EXISTS (SELECT 1 FROM tb_projects WHERE title = 'Análise de Estoque');

-- Projeto CANCELLED
INSERT INTO tb_projects (title, description, client_id, status, start_date, expected_end_date, end_date, value)
SELECT 'Monitoramento de Máquinas', 'Sistema de monitoramento IoT integrado ao BI', 5, 'CANCELLED', '2025-03-01', '2025-07-01', null, 30000.00
